...
Пропустити крок — `--keep-settings`.

**Віртуальні принтери в локальній копії вимикаються за замовчуванням.** Їхні
адреси — IP-аліаси на хості прода (той самий MAC, що й у Home Assistant), тож
у dev вони не забіндяться ніколи й лише сиплють ~40 помилок на кожному старті.
Лишити ввімкненими — `--keep-vp`. Окремо `--no-auth` вимикає логін: зручно, але
тоді будь-який пристрій, що дістає цю інстанцію — а з tailnet-сайдкаром це вся
мережа tailnet — керує реальними принтерами без пароля.
