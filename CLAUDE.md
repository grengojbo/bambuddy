...
Пропустити крок — `--keep-settings`. Два прапорці міняють локальну копію на вимогу:
`--no-auth` вимикає логін (зручно, але тоді будь-який пристрій, що дістає цю
інстанцію — а з tailnet-сайдкаром це вся мережа tailnet — керує реальними
принтерами без пароля), `--no-vp` глушить віртуальні принтери. Другий варто
вмикати майже завжди: адреси VP — це IP-аліаси на хості прода (той самий MAC,
що й у Home Assistant), тож у dev вони не забіндяться ніколи й лише засипають
лог помилками на кожному старті. Автоматичне скачування
